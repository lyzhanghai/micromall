...
public interface CartGoodsMapper extends BaseMapper<CartGoods> {
	...
	@Select("SELECT t2.id goodsId, t2.title, t2.main_picture as picture, t2.price, t1.buy_number as buyNumber, t2.inventory FROM cart_goods t1 RIGHT"
			+ " JOIN goods t2 ON t1.goods_id = t2.id WHERE t1.uid = #{uid} ORDER BY t1.id DESC")
	@ResultType(CartGoodsDTO.class)
	java.util.List<CartGoodsDTO> list(int uid);
